...
#ifdef CONFIG_FIH_SW3_POWER_ON_REASON
...
static int gbattery_test[BATTERY_TEST_NUMS] = {-1};
#define BATTERY_TEST_MAGIC  0x0FAF0F0F

static struct msm_battery_gauge *msm_batt_gauge = NULL;
/* FIH-SW3-KERNEL-VH-CHARGING-12+ */
static struct wake_lock msm_battery_wakelock;
static struct wake_lock msm_battery_ac_wakelock;


/* FIH-SW3-KERNEL-VH-CHARGING-10*[ */
...
extern int msm_chg_usb_charger_connected(uint32_t type);
extern int msm_chg_usb_i_is_available(uint32_t sample);
extern int msm_chg_usb_i_is_not_available(void);
extern int msm_chg_usb_charger_disconnected(void);

static ssize_t msm_battery_ftm_charging_write(struct device *dev, 
        struct device_attribute *attr, const char *buf, size_t count)
{
    int cmd     = 0;
    int data[3] = {0, 0, 0};
    static int isCharging = 0;
    static int chg_current = 0;
    int retVal = 0;
    
    sscanf(buf, "%d %d %d %d", &cmd, &data[0], &data[1], &data[2]);
    dev_err(dev, "%s: COMMAND: %d data1 = %d data2 = %d\n", __func__, cmd, data[0], data[1]);
        ...
        if (data[0] == 0) {
            ...
            if (isCharging) {
                ...
                if (chg_current == 0) {
                    ...
                    /* Workaround: Modem charger will decrease 50mA by default foe USB IF */
                    msm_chg_usb_i_is_available(chg_current+50);
                    isCharging = 1;
                } else {
    ...
    dev_err(dev, "%s: isCharging=%d, chg_current=%d\n", __func__, isCharging, chg_current);
    return retVal;
}

static ssize_t msm_battery_ftm_charging_read(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "0\n");
}

static DEVICE_ATTR(ftm_charging, 0644, msm_battery_ftm_charging_read, msm_battery_ftm_charging_write);
#endif
/* FIH-SW3-KERNEL-VH-CHARGING-10*] */
...
/* FIH-SW3-KERNEL-VH-CHARGING-14*[ */
/* Remove static to extern to bq27520 */
void msm_batt_update_psy_status(void)
{
	...
	if (msm_batt_get_batt_chg_status()) {
        wake_unlock(&msm_battery_wakelock);
		return;
    }
    ...
    if (gbattery_test[BATTERY_TEST_TEMPERATURE] != BATTERY_TEST_MAGIC) {
        battery_temp = gbattery_test[BATTERY_TEST_TEMPERATURE];
        DBG_LIMIT("TEMP from test\n");
    }
    /* FIH-SW3-KERNEL-EL-fix_temp_read_from_PMIC-00*[ */
    else if (msm_batt_gauge && msm_batt_gauge->get_battery_temperature) {
        battery_temp = msm_batt_gauge->get_battery_temperature();
    } else {
        battery_temp = rep_batt_chg.v1.battery_temp;
        DBG_LIMIT("TEMP from Modem\n");
    }

    /* FIH-SW3-KERNEL-EL-fix_temp_read_from_PMIC-00*] */
    if (msm_batt_gauge && msm_batt_gauge->get_battery_mvolts) {
        battery_voltage = msm_batt_gauge->get_battery_mvolts();
    } else {
        battery_voltage = rep_batt_chg.v1.battery_voltage;
        DBG_LIMIT("Voltage from Modem\n");
    }

    if (msm_batt_gauge && msm_batt_gauge->get_battery_flags) {
        battery_flags = msm_batt_gauge->get_battery_flags();
    }

    if (gbattery_test[BATTERY_TEST_CP] != BATTERY_TEST_MAGIC) {
        batt_capacity = gbattery_test[BATTERY_TEST_CP];
        DBG_LIMIT("CP from test\n");
    } else if (msm_batt_gauge && msm_batt_gauge->get_batt_remaining_capacity) {
        batt_capacity = msm_batt_gauge->get_batt_remaining_capacity();
    } else {
        batt_capacity = msm_batt_info.calculate_capacity(battery_voltage);
	...
	if (msm_batt_info.charger_type != charger_type) {
		if (charger_type == CHARGER_TYPE_USB_PC||
			...
			supp = &msm_psy_usb;
			bq27520_change_polling_timeout(BATT_POLLING_STATUS_CHARGING);
			...
			supp = &msm_psy_ac;
			bq27520_change_polling_timeout(BATT_POLLING_STATUS_CHARGING);
			wake_lock(&msm_battery_ac_wakelock); 
		} else {
			...
			bq27520_change_polling_timeout(BATT_POLLING_STATUS_DISCHARGING);
			wake_unlock(&msm_battery_ac_wakelock); 
	...
	if (msm_batt_gauge) {
		#define MAX_LOWER_CNT_CHG 10	/* the interval would be between MAX_LOWER_CNT * (30s ~ 60s) */
		#define MAX_LOWER_CNT_DISCHG 5

		static int		lower_than_3p5_cnt = 0;
		int			lower_than_3p5_cnt_max;
		...
		if (battery_voltage <= 3500) {
			...
			if (lower_than_3p5_cnt >= lower_than_3p5_cnt_max)
				batt_capacity = 0;
		}
		else {

			lower_than_3p5_cnt = 0;

			/* if battery voltage greater than 3.5V, and CP == 0, force CP to be 1*/
			if (batt_capacity == 0)
				batt_capacity = 1;
		}
	}
    ...
    if (batt_capacity > 100) {
        DBG_LIMIT("BATT: Error capacity read, CP = %d.\n", batt_capacity);		
        batt_capacity = msm_batt_info.batt_capacity;		
    }

    if ((msm_batt_info.batt_status == POWER_SUPPLY_STATUS_DISCHARGING) && 
				(batt_capacity < 10) ) {
		if (bq27520_get_polling_timeout() == BATT_POLLING_STATUS_DISCHARGING) {
			bq27520_resch_change_polling_timeout(BATT_POLLING_STATUS_CHARGING, true);
...
#endif
	/* FIH-SW3-KERNEL-VH-CHARGING-12+ */
	wake_lock_destroy(&msm_battery_wakelock);
	wake_lock_destroy(&msm_battery_ac_wakelock);
			    ...
			    void *buffer, int in_size)
{
	...
	u32 procedure;
	u32 accept_status;
	wake_lock(&msm_battery_wakelock); 
	...
	if (accept_status == RPC_ACCEPTSTAT_SUCCESS) {
		/* 	Here don't call wake_unlock explicitly, msm_batt_update_psy_status 
			invoked by delay work queue will do it in msm_batt_update_psy_status.
			ATTENTION: DON'T remove wake_unlock statement in 
			msm_batt_update_psy_status for the above reason.
		*/
		bq27520_resch_change_polling_timeout(0, 0);
	} else {
		wake_unlock(&msm_battery_wakelock); 
	}
...
static ssize_t chg_is_power_key_pressed(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int retVal = 0;

    proc_comm_is_pwr_key_pressed(&retVal);

    return snprintf(buf, PAGE_SIZE, "%d\n", retVal);

    return 0;
}
static DEVICE_ATTR(isPowerKeyPressed, 0644, chg_is_power_key_pressed, NULL);
static ssize_t chg_is_charger_connected(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    int isConnected = 0;

    proc_comm_is_charger_connected(&isConnected);
...
static DEVICE_ATTR(isChargerConnected, 0644, chg_is_charger_connected, NULL);
...
static ssize_t chg_is_enter_power_off_charging(struct device *dev,
        struct device_attribute *attr, char *buf)
{
#ifndef CONFIG_FIH_SW3_POWER_OFF_CHARGING
	return snprintf(buf, PAGE_SIZE, "0\n");
#endif

#ifdef CONFIG_FIH_SW3_POWER_ON_REASON
    if (fih_get_power_on_cause() == PWR_ON_EVENT_USB_CHG)
        return snprintf(buf, PAGE_SIZE, "1\n");
    else
        return snprintf(buf, PAGE_SIZE, "0\n");
...
static DEVICE_ATTR(isEnterPowerOffCharging, 0644, chg_is_enter_power_off_charging, NULL);

static ssize_t chg_is_enable_charging(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    chg_ipc_smem *pChgIpcInfo;
    pChgIpcInfo = (chg_ipc_smem *) smem_alloc(SMEM_BATT_INFO, sizeof(*pChgIpcInfo));

    return snprintf(buf, PAGE_SIZE, "%d\n", pChgIpcInfo->parameters[CHG_IPC_PARAMETER_FLAG]);
}
static DEVICE_ATTR(isEnableCharging, 0644, chg_is_enable_charging, NULL);

static ssize_t chg_is_battery_full(struct device *dev,
        struct device_attribute *attr, char *buf)
{
...
static DEVICE_ATTR(isBatteryFull, 0644, chg_is_battery_full, NULL);
#endif

static void battery_test_init(void)
{
    int i = 0;
	
    for (i = 0;i < BATTERY_TEST_NUMS;i++) {
        gbattery_test[i] = BATTERY_TEST_MAGIC;
    }
}

static ssize_t battery_test_capacity_read(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%d\n", gbattery_test[BATTERY_TEST_CP]);
}

static ssize_t battery_test_capacity_write(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int val = 0;
    
    sscanf(buf, "%3d", &val);
    dev_info(dev, "%s: Set CP to %d\n", __func__, val);
    gbattery_test[BATTERY_TEST_CP] = val;
    msm_batt_update_psy_status();

    return count;
}
static DEVICE_ATTR(test_capacity, 0644, battery_test_capacity_read, battery_test_capacity_write);
static ssize_t battery_test_temperature_read(struct device *dev,
        struct device_attribute *attr, char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%d\n", gbattery_test[BATTERY_TEST_TEMPERATURE]);
}

static ssize_t battery_test_temperature_write(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    int val = 0;
    
    sscanf(buf, "%4d", &val);
    dev_info(dev, "%s: Set Temperature to %d\n", __func__, val);
    gbattery_test[BATTERY_TEST_TEMPERATURE] = val;
    msm_batt_update_psy_status();

    return count;
}
static DEVICE_ATTR(test_temperature, 0644, battery_test_temperature_read, battery_test_temperature_write);

static struct attribute *fs_attrs[] = {
	&dev_attr_test_capacity.attr,
    &dev_attr_test_temperature.attr,
	NULL,
};
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	/* FIH-SW3-KERNEL-VH-CHARGING-12+ */
	wake_lock_init(&msm_battery_wakelock, WAKE_LOCK_SUSPEND, "msm_battery");
	wake_lock_init(&msm_battery_ac_wakelock, WAKE_LOCK_SUSPEND, "msm_battery_ac");
...
#ifndef CONFIG_BATTERY_MSM_FAKE
    /*FIH-SW3-KERNEL-VH-CHARGING-09* VBATT_CHG_ALL -> VBATT_CHG_EVENTS */
	rc = msm_batt_register(BATTERY_LOW, VBATT_CHG_EVENTS,
			       BATTERY_CB_ID_ALL_ACTIV, VBATT_CHG_EVENTS);
	if (rc < 0) {
		dev_err(&pdev->dev,
...
#ifdef CONFIG_FIH_SW3_POWER_OFF_CHARGING
    rc = device_create_file(&pdev->dev, &dev_attr_isPowerKeyPressed);
    if (rc) {
        dev_err(&pdev->dev,
               "%s: dev_attr_isPowerKeyPressed failed\n", __func__);
    }
    rc = device_create_file(&pdev->dev, &dev_attr_isChargerConnected);
    if (rc) {
        dev_err(&pdev->dev,
               "%s: dev_attr_isChargerConnected failed\n", __func__);
    }

    rc = device_create_file(&pdev->dev, &dev_attr_isEnterPowerOffCharging);
    if (rc) {
        dev_err(&pdev->dev,
               "%s: dev_attr_isEnterPowerOffCharging failed\n", __func__);
    } 

    rc = device_create_file(&pdev->dev, &dev_attr_isEnableCharging);
    if (rc) {
        dev_err(&pdev->dev,
               "%s: dev_attr_isDisableCharging failed\n", __func__);
    } 
    
    rc = device_create_file(&pdev->dev, &dev_attr_isBatteryFull);
    if (rc) {
        dev_err(&pdev->dev,
               "%s: dev_attr_isBatteryFull failed\n", __func__);
    } 
#endif

    rc = sysfs_create_group(&pdev->dev.kobj, &fs_attr_group);
    if (rc) {
        dev_err(&pdev->dev,
...
static int __devinit msm_batt_init_rpc(void)
{
	...
	if ((s32)msm_batt_info.chg_api_version < 0)
		msm_batt_info.chg_api_version = CHG_RPC_VER_1_1;
