...
#define DRIVER_NAME	"fih_power-key"
...
#define HS_HEADSET_SWITCH_4_K	0xF2/* MM-RC-HEADSET-LiveKEY-DETECT+ */
...
extern void button_pressed(int pkey);
extern void button_released(int pkey); 
 /* MM-RC-HEADSET-MULTIBUTTON-DETECT+] */
enum hs_event {
...
static const uint32_t hs_key_map[] = {
	...
	KEY(HS_HEADSET_SWITCH_4_K, BTN_3),/* MM-RC-HEADSET-LiveKEY-DETECT+ */
	0
};
...
static struct fih_power_key_data	*power_key_data;
...
int	get_interval( unsigned long old, unsigned long new )
{

	return	old > new ? LONG_MAX - old + new : new - old;
...
static void report_hs_key(uint32_t key_code, uint32_t key_parm)
{
	...
	case KEY_VOLUMEUP:
	case KEY_VOLUMEDOWN:
       case BTN_3: /* MM-RC-HEADSET-LiveKEY-DETECT+ */
  ...
  /* MM-RC-HEADSET-MULTIBUTTON-DETECT+] */
	...
	case KEY_END:
		if( key == KEY_POWER || key == KEY_END )
		{
			...
			counter	= get_interval( power_key_data->tick_time, jiffies );

			if( !power_key_data->set_interval && ( counter < msecs_to_jiffies( power_key_data->interval ) ) )
			{

				if( key_code != HS_REL_K )
					atomic_inc( &power_key_skip_count );

				printk( "PKEY : Skip Power-Key event(%d)\n", atomic_read( &power_key_skip_count ) );

				break;

			}

			if( power_key_data->set_interval )
				power_key_data->set_interval	= 0;

			if( key_code != HS_REL_K )
			{

				down( &tick_time_protection );

				power_key_data->tick_time		= jiffies;

				up( &tick_time_protection );

				power_key_data->set_interval	= 1;
...
static ssize_t power_key_control( void *node_data, struct device_attribute *attr, const char *buf, size_t size )
{

	struct fih_power_key_data	*data	= node_data;

	int	ID, interval;

	get_para_from_buffer( buf, "%d%d", &ID, &interval );

	if( ID != SET_INTERVAL )
		printk( "PKEY : Dose not support this ID(%d)\n", ID );
	else
	{

		data->interval	= interval;

		printk( "PKEY : Set interval(%dms)\n", data->interval );
...
static ssize_t power_key_info( void *node_data, struct device_attribute *attr, char *buf)
{
	struct fih_power_key_data	*data	= node_data;

	return	snprintf( buf, PAGE_SIZE, "Interval = %dms\n", data->interval );
}

void	check_power_key_skip_count( void )
{
	...
	if( skip_count % 2 )
	{
		...
		printk( "PKEY : Skip count(%d), send Power-key event\n", skip_count );

	}

	atomic_set( &power_key_skip_count, 0 );

	down( &tick_time_protection );

	power_key_data->tick_time	= jiffies - msecs_to_jiffies( power_key_data->interval );

	up( &tick_time_protection );

}

EXPORT_SYMBOL(check_power_key_skip_count);
...
static int __devinit hs_probe(struct platform_device *pdev)
{
	...
	power_key_data	= kzalloc( sizeof( struct fih_power_key_data ), GFP_KERNEL );

	if( !power_key_data )
	{
		printk( "PKEY : Failed to allocate state\n" );
		return	-ENOMEM;
	}

	power_key_data->interval		= ( ( struct msm_handset_platform_data* )pdev->dev.platform_data )->interval;
	...
	// Create file node in sys/class/power_key/fih_power_key
	file_node.class_name	= "power_key";

	file_node.device_name	= "fih_power_key";

	file_node.file_node_data	= power_key_data;

	file_node.control_read	= 0;

	file_node.control_write	= power_key_control;

	file_node.info_read	= power_key_info;
